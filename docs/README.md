...
- **Foundations** -- short vocabulary/concept cards (`flashcards/foundations.json`),
  ...
  The Practice level (level 2, bridge multiple-choice cards) is populated
  (scenario MCQs using only vocabulary already taught, plus a small MPEP
  Chapter Map topic for chapter-recall drills) and selectable like any
  other level.
