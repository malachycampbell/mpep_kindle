...
- **`reference/historical_exams/`** -- the 3 USPTO registration-exam sessions ever officially released (Oct 2002, Apr 2003, Oct 2003; recovered via the Wayback Machine, since USPTO's original hosting is gone), plus `extract.py` and its structured JSON output in `extracted/`. These predate the AIA and current MPEP, so content here is **not** assumed current law until audited -- see `audit/METHODOLOGY.md` and `audit/2002-10-16_audit.json` / `audit/2003-04-15_audit.json` (2 of 3 sessions audited so far).
...
| 2 -- Historical exam extraction & current-law audit | Extraction done; audit in progress (2 of 3 sessions) |
